...
contract StrikersPackFactory is Pausable {
  ...
  // which may be less than we need for any given sale, so we do it in batches. The Standard Sale runs all tournament long, and we
  ...
  function startStandardSale() external onlyOwner {
    ...
    unpause();
    emit SaleStarted(0, standardSale.packPrice, 0);
  }
  ...
  function createNextPremiumSale(uint8 _featuredChecklistItem, uint256 _packPrice) external onlyOwner nonZeroPackPrice(_packPrice) {
    ...
    nextPremiumSale.packPrice = _packPrice; // set packprice
    saleCount++;
  }
